...
public class NextNumber {
     ...
     * When we do that, the number will be bigger if and only if
     * the zero-to-one bit was to the left of the one-to-zero bit
     * We want to make the number bigger, but not unnecessarily bigger.
     * Therefore, we need to flip the right most zero which has ones on the right of it
     * AKA, we're flipping the rightmost non-trailing zero.
     */

    public static int getNext(int n) {
        int c = n;
        int c0 = 0; // the number of 0s to the right of the first non-trailing zero
        int c1 = 0; // the number of 1s to the right of the first non-trailing zero

        while (((c & 1) == 0) && (c != 0)) {
            c0++;   // counting number of trailing zeros
            c >>= 1;    // Arithmetic shift by one
        }

        while (((c & 1) == 1)) {
            c1++;   // counting number of 1s to the right of non-trailing zero
            c >>= 1;
        }
        ...
        // position of right-most non-trailing zero (where the right most bit is bit 0)
        int pos = c0 + c1;
    ...
    public static int getPrev(int n) {
        int c = n;
        int c0 = 0; // the number of 0s to the right of non-trailing zero
        int c1 = 0; // the number of 1s to the right of non-trailing zero
